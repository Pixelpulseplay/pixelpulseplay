...
const SHEET_URL = `https://docs.google.com/spreadsheets/d/1JRp6yhLKJuvZL9uCmYi-ipKPHQ7bdJcB/export?format=xlsx`;
...
async function fetchsheetdata(sheetName, location) {
  ...
  const shouldBypassCache = sheetName === 'promotions';
  ...
  if (!shouldBypassCache && cached && now - cached.timestamp < CACHE_TTL) {
